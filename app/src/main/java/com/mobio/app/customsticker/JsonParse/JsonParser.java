...
import com.mobio.app.customsticker.Utils.ProgressBarDialogue;
...
public class JsonParser {
    ...
    public JsonParser(Context context, Activity activity, String progressMessage) {
    ...
    public void userPostRequest(int offsetData, int dataLimit, String URL, String imgID, String catID, final CallBackInterface callBackObj) {
        ...
        progressBarDialogue = new ProgressBarDialogue(context);
        ...
        httpClient.post(activity, URL, params, new AsyncHttpResponseHandler() {
            ...
            public void onStart() {
                ...
                if (Values.progressFlag == Values.bigProgressBar) {
                    progressBarDialogue.showProgressDialogue(progressMessage);
                }
            }

            @Override
            public void onSuccess(int arg0, org.apache.http.Header[] headers, byte[] response) {
                try {
                    ...
                    if (jsonObject.has("images")) {
                        ...
                        for (int i = 0; i < jsonArrayLength; i++) {
                            ...
                            String ThambnailSmallImageURL = jsonObjectLocal.getString("thumnail_url");

                            StickerModelClass aSticker = new StickerModelClass(id, banglaCaption, ThambnailSmallImageURL);

                            stickerDB.writeSingleStickerData(aSticker);
                        }

                        //this portion only created to increase default counter of all data------ starts
                        if (progressBarDialogue != null && Values.progressFlag == Values.bigProgressBar) {
                            sharedPref.setDATA_COUNTER(sharedPref.getDataCount() + jsonArrayLength);
                        }
                        //for category wise sticker
                        Values.showLog(Values.ERROR, "JsonParser", "jsonArrayLength=" + jsonArrayLength);
                       /* if(Values.progressFlag==Values.smallProgressBarr && jsonArrayLength<8){
                            sharedPref.setCategoryWiseStickerAvailableInServer(false);
                        }*/
                        /*if(jsonArrayLength<49){
            ...
            public void onFailure(int i, org.apache.http.Header[] headers, byte[] bytes, Throwable throwable) {
                //userPostRequest(offsetData,dataLimit ,URL);
